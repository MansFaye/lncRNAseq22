...
## example of how to find intersection between two files (here CAGE peaks and single-exon transcripts)
if [ 1 == 1 ]
then
	...
	sort $output_dir/cage_transcript_ids.txt $assembly_dir/single_exon_tx.txt | uniq -d > $output_dir/cage_x_single_exons.txt
fi

# Evaluate Protein-coding potential (ORFs)
## generate a BED file of novel transcripts from our annotation 

awk -F $'\t' '$3=="transcript"' $assembly_dir/merged.gtf | grep -v gene_name | awk -F $'\t' 'BEGIN { OFS="\t" }{print $1, $4, $5, $9, ".", $7, ".", ".", ".", ".", ".", "."}' | grep '^chr' > $cpat_dir/merged_novel.bed

## run cpat

cpat.py -x $cpat_dir/Human_Hexamer.tsv -d $cpat_dir/Human_logitModel.RData -g $cpat_dir/merged_novel.bed -r $quantif_dir/transcripts.fa -o $cpat_dir/output
